...
export const getTimeCapsule = () => {
  ...
  todayStartDate.setHours(0, 0, 0, 0);  // 当天的零点时间
...
export const siteDateStatistics = (startDate) => {
  const currentDate = new Date();
  const differenceInTime = currentDate.getTime() - startDate.getTime();  // 计算毫秒差
  const differenceInDays = Math.round(differenceInTime / (1000 * 3600 * 24));  // 转换为天数
  
  return `本站已经苟活了 ${differenceInDays} 天`;
};
